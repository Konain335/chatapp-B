...
export const getcurrentChatters = async (req, res) => {
  try {
    const currentUserID = req.user._id.toString(); // string
    ...
    const partcipantsIDS = currentChatters.reduce((ids, conversation) => {
      const otherParticipents = conversation.participants
    ...
    const uniqueOtherIDs = [...new Set(partcipantsIDS)];

    const usersList = await User.find({ _id: { $in: uniqueOtherIDs } })
